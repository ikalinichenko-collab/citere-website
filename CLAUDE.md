...
`docs/citere-spec/` holds the governing brief (`CLAUDE_CODE_BRIEF.md`), ten reference HTML prototypes, and the specifications. Where the brief and this file disagree about *what pages exist and what a figure means*, the brief wins; where a demo and a spec disagree, the spec wins; this file still governs the stack, the prose rules and the pre-publish checks. Work through the brief's §6 "Order of work"; §13 below describes the build that came before it.

The normative documents for every number are `Citere_Calculation_Methodology.md` (CM) — "any deviation in code or reporting is a defect, not a variant" — `Citere_Entity_Model_and_Field_Instructions.md`, `Citere_Sources_Registry_Business_Logic.md` (SR), `Citere_Claim_Report_Spec.md`, `Citere_Countries_Pages_Description.md` and `Citere_pipeline_implementation_guide.md`.

Landed so far (step 1):

- **The demo dataset is the grid the specifications define.** 9 claims × 4 personas × 4 variations × 6 bots × 3 repeats = 2,592 responses per run (CM §1.1), across 5 markets — US/EN, DE/DE, AT/DE, UA/UK, FR/FR — with US/EN measured twice so trend, citation drift and cleansing have something to read. Generated by `seed-data/generate_grid.py`. A claim-level cell is n = 12 and renders greyed; pooled over the cluster it is n ≈ 108, which is where significance is tested.
- **Responses live in `data/observations/{run_id}.csv`, not inside the claim cards.** A claim card is a card. `data/runs.json` is the source run table (grid shape, frozen versions); `data/metrics.json` is derived — one cell per bot × claim × persona × market × run, plus the per-run aggregates — by `scripts/build-metrics.mjs` (`npm run metrics`). `npm run validate` re-derives it, fails on drift, and asserts its arithmetic (SR §12).
- **The rules are in the code, not in the discipline of each page.** `src/_data/metrics.js`: counts may be summed across claims and bots; a *rate* only forms inside one persona, one market and one run, otherwise it returns `null` with the reason in `blocked` (CM §5.1). Two denominators, never interchanged (CM §5.2–5.5). `quarantine` and `UNRESOLVED` are counted and then excluded (CM §0.3). Wilson at z = 1.96, `low_n` below 20, significance only by non-overlap (CM §5.6). The grain-of-truth split runs on `splice`, never on the `grain_of_truth` field (CM §5.7). Live formulations are a separate cell and are never pooled with the grid.
- **Layer B.** Watchlist matching is exact or subdomain on a dot boundary, with a mandatory exclusion list — `pravda.com.ua` is Ukrainska Pravda and must never match (CM §3.2, pipeline guide §4.1). Networks fold onto `pravda_network > state_media > laundering_network`. One response counts once per domain (CM §5.8). Every domain edge carries its verdict split and critical count, which is what the Sources Registry reads (SR §5).
- **Escalations → Countermeasures.** `/countermeasures/`, `data/countermeasures.json`, `countermeasures.csv`; `/escalations/` is a redirect stub. The twelve types of the catalogue §2.1–2.12, the one lifecycle `drafted → … → declined`, and the §4 ladder live in `src/_lib/labels.cjs`. **A re-measurement is not a type**: the catalogue puts it inside the disclosure lifecycle (§2.1 "Track"), so it is carried as `kind: "remeasurement"` — its own column wherever a page asks for one, never counted as a countermeasure taken. `submission_content`, `proof`, `target_contact` and `confirmed_by` are internal: the schema forbids them in `data/`, and `check.mjs` fails if they reach the build.
- **The Claim Report is built to `Citere_Claim_Report_Spec.md`.** Three layers in the spec's order: the finding as the headline with a lede generated from the run; "Why this is false", the assistant-by-assistant account and what it shows, in counts only; then evidence, the claim card, the sources identified, the per-market tables with intervals, the A×B matrix, live formulations against the grid, what changed, countermeasures, limitations generated from the run, and the method summary.
- **Benchmarks is a shell, and `data/benchmarks.json` is gone.** `src/_data/benchmarks.js` now derives every figure from the metrics store inside one run, and the caption names that market and month. `/benchmarks` lists the issues — one cluster × one month — and carries the two figure sets the brief says the page will absorb: the grain-of-truth split (on `splice`, per CM §5.7, which the precomputed file got wrong) and the escalation matrix. `/benchmarks/{cluster}-{yyyy-mm}/` is the issue: coverage with frozen versions, the matrix and the split per market, and the claims tested.
- **Chatbot and market pages.** `/platforms/{bot}` carries the verdict distribution and the retrieval/contamination split per market and question type, the domains it cites, the citation-drift baseline from `data/citation-drift.json`, the market pairs whose intervals do not overlap, and its own countermeasure ladder. `/countries` generates its headline from the largest significant gap and renders each finding only if the comparison clears significance — where it does not, the block says so. `/countries/full` is the breakdown for anyone who needs every figure.
- **The countermeasures log and the per-claim escalation report.** `/countermeasures` is the flat log: six tiles including "awaiting your confirmation", every filter of the brief's §4 as a static URL under `/countermeasures/{type,status,target,market,claim}/{value}/`, and a row that opens to its evidence package and the honest difference between a draft and a captured submission. `/registry/{slug}/escalation/` carries the catalogue §4 ladder computed from the actions actually logged; a locked rung names the prerequisite it is waiting on, and the terminal rung stays locked until the platform has been disclosed to, the finding is on the public record, and a re-measurement has found no remediation (catalogue §2.12, CM §7).
- **The sources registry is built to `Citere_Sources_Registry_Business_Logic.md`.** `/sources` is the §10.1 table — citations, assistants, markets, claims reached, source-to-answer lines, critical incidents — with the §10.1 filters as static URLs under `/sources/{network,category,chatbot,market,flag}/{value}/`. A domain page carries the claims it carried, the ones it reached without carrying, the bot × persona grid, its source-to-answer lines, the market split and the per-run timeline. An unclassified domain is internal (§11): it stays out of the index and gets no page, though it still appears on a claim card as a surface object.
- **Generated response text is labelled where it is shown.** The demo carries a prompt and a response quote for CRITICAL and HIGH incidents only, built by template from the claim card by `seed-data/generate_grid.py` following the persona × splice matrix (Entity Model V.4), in the language of the market. Each carries `synthetic: true`, and the Evidence block prints the notice beside the quotes rather than relying on the demo banner at the top. `check.mjs` fails if a `data-synthetic="true"` block appears in a section without `data-notice="synthetic-quotes"`. P4 prompts are stored as a paraphrase and never verbatim (CLAUDE.md §11.6).
- **A between-run delta never stands alone.** `src/_includes/partials/cleansing.njk` is the render rule: every delta goes through `deltaCell()` and every block ends with `deltaCaveats()`, which prints the no-control-group caveat (CM §7.6 — "observed change after", never "effect of") and the citation-drift caveat from `data/citation-drift.json` (an external figure: roughly half an assistant's cited domains change month to month, so a significant delta is not on its own evidence of a fix). `check.mjs` enforces it two ways — every `data-delta="cleansing"` must have both `data-caveat` markers in the same section, and independently every percentage-point figure in the output must too, so a delta printed without the macro is still caught. The phrase "effect of the escalation" fails the build.
- **Clusters are not navigation.** `cluster` survives only as a field on a claim.

Open against the specs:

- `Citere_pipeline_implementation_guide.md` is from the earlier project and describes the processing pipeline, not the site. Treat it as context; where it disagrees with the 2026-09-09 specifications, they win.
- `flagged_truth_as_false` is in the schema and never populated, by design (Entity Model §7). `debunk_argument` is written for three claims only; the rest render the section's empty state.
- The pages are still the pre-restructure set. Steps 2–7 of the brief rebuild them: the Registry and Claim Report, then Sources, Countermeasures and the per-claim escalation report, then the chatbot and country pages, then `/data`, then the Benchmarks shell.
...
- **Page weight target:** HTML ≤ 60 KB per page; a Claim Report gets 100 KB, because the spec requires three layers, an evidence block per flagged answer, a bot × persona table per market, the A×B matrix and the limitations, and none of that is optional. A build transform strips template indentation from the output. Lighthouse ≥ 95 in all four categories. The Lighthouse step must build into its own directory and never overwrite `_site/`.
...
- **Terminology guard**: cited domains are "listed" or "watchlisted", never "Kremlin-linked".
- **Between-run delta guard**: no percentage-point change may be rendered without the no-control-group and citation-drift caveats in the same section, and no page may claim an "effect of escalation".
- **Generated-text guard**: a block carrying generated response text must sit in a section that says so.
